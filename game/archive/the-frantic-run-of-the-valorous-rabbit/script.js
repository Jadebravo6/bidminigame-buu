...
function sendScoreToServer(score) {
  // Supposons que le serveur expose une API pour recevoir les scores
  const serverURL = 'http://localhost:3000/api/scores';

  // Données à envoyer au serveur (dans cet exemple, un objet JSON avec le score)
  const data = {
    score: score,
  ...
  fetch(serverURL, options)
    .then(response => {
      if (!response.ok) {
        throw new Error('Erreur lors de l\'envoi du score au serveur');
      }
    ...
    .then(responseData => {
      ...
      console.log('Score envoyé avec succès', responseData);
    })
    .catch(error => {
...
function gameOver() {
  ...
  finalScore = Math.floor(distance / 2);

  // Ajouter le nom du jeu à l'objet data
  const data = {
    
    gameName: "the-frantic-run-of-the-valorous-rabbit",
    score: finalScore
    
    // Ajoutez d'autres données si nécessaire
  };

  // Appeler la fonction pour envoyer le score au serveur
  sendScoreToServer(data);
}
