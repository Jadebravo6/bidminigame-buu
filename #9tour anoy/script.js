$(document).ready(function() {
   ...
   // Redémarre le jeu et cache la modal
   $('#reload-btn').on('click', function() {
    initGame($tower.eq(0)); // Restart the game
    ...
    initialStyle.setAttribute('data-custom-style', 'true');
    ...
    // Cacher la modal de victoire après le redémarrage
    $('#win-modal').fadeOut(500);
});




// Quitter le jeu depuis la modal de victoire
$('#quit-btn').on('click', function() {
    quitGame(); // Quit the game
    $('#win-modal').fadeOut(500); // Cacher la modal de victoire après le quit
});
...
$('#quit-button').on('click', function() {
    quitGame(); // Quit the game
 // Cacher la modal de victoire après le quit
});
// Redémarre le jeu et cache la modal de confirmation de redémarrage
$('#confirm-restart').on('click', function() {
    ...
    // Cacher la modal de redémarrage après confirmation
    $('#restart-modal').fadeOut(500);
});

// Retour à la page d'accueil depuis la modal de redémarrage
$('#home-btn').on('click', function() {
    quitGame(); // Quit the game
    $('#restart-modal').fadeOut(500); // Cacher la modal de redémarrage après retour à l'accueil
});
    ...
    // Click handler to prevent default action
    $('#win-modal').on('click', function(event) {
        event.preventDefault();
        // Aucune autre action ici
    ...
    const victorySound = new Audio('sounds/complet.wav');
    ...
    function startTimer() {
        ...
        timerInterval = setInterval(function() {
            ...
            // Si le timer atteint 30 secondes, commencer à jouer le son à chaque seconde
            if (timerSeconds <= 30 && timerSeconds > 0) {
                beepSound.play(); // Jouer le son à chaque seconde quand timerSeconds <= 30
            }
    
            // Changer le fond quand le timer atteint 30 secondes
            if (timerSeconds === 30) {
                $('body').css('background', 'linear-gradient(to bottom, #b63c17, #1d0d08)');
            }
    
            // Si le timer atteint 0, arrêter le timer et afficher le modal de défaite
            if (timerSeconds <= 0) {
                clearInterval(timerInterval);
                $('#restart-modal').fadeIn(500); // Afficher le modal de défaite
            }
        }, 1000); // Exécuter cette fonction chaque seconde
    }
    ...
    function countMove() {
        ...
        if (moves > minMoves - 1) {
            if ($tower.eq(1).children().length === disksNum || $tower.eq(2).children().length === disksNum) {
                ...
                victorySound.play(); // Jouer le son de victoire
