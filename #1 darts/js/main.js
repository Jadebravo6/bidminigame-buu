(function(window, document, PIXI, Charm, $) {
  ...
  var dartboard,
    ...
    //la variable qui attribue les points 
  ...
  function startGame() {
    ...
    // Réinitialiser l'affichage du score à zéro
  ...
  function startXAxis() {
    lineX.visible = true;
    lineXTween = c.slide(lineX, BOARD_SIZE, 0, 60, 'smoothstep', true);
    in_x_axis = true;
  }
  ...
  function startYAxis() {
    in_x_axis = false;
    lineY.visible = true;
    lineYTween = c.slide(lineY, 0, BOARD_SIZE, 60, 'smoothstep', true);
    in_y_axis = true;
  }
  ...
  function throwDart() {
    ...
    // la viteste-sse par defaut est 20 
    c.scale(dart, 1, 1, 20);
  ...
  function checkThrow(x, y) {
    ...
    if (score > 0) {
        if (score === 50) { // Si le score est de 50 (centre de la cible)
            message = 'JACKPOT!';
    
            textStyle = { // Style de texte pour le jackpot
                fontFamily: 'Arial',
                fontSize: '60px',
                fontStyle: 'italic',
                fontWeight: 'bold',
         ...
         } else if (score === 25) { // Si le score est de 25
            message = score; // Message spécifique pour le secteur extérieur
            score = 25; // Attribuer la valeur 25
  ...
  function end() {
    ...
    var text = new PIXI.Text(' Bids gagné  :', textStyleMiss);
    text.x = BOARD_SIZE / 2;
